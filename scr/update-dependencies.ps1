...
.DESCRIPTION
    KoreForge.Json depends on KoreForge.Jex (a sibling KoreForge package).
    The default version pin is stored as a fallback in Directory.Packages.props.
    This script updates that pin to match the version being released so that
    dotnet pack produces a stable (non-prerelease) package reference.
